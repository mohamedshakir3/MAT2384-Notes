...
\[Y = Y_H + Y_p\]
Where $Y_H$ is the general solution to the corresponding homogeneous system $Y' = AY$, and $Y_p$ is the particular solution to the non-homogeneous system $Y' = AY + \overrightarrow{r(x)}$. To find $y_p$, we use the method of undetermined coefficients. The constants in the undetermined coefficients method are replaced by constant vectors. For example, if 
\[\overrightarrow{r(x)} = \begin{bmatrix}
    2\cos x + x^2 - x \\
    3e^x - x^2 + 2x + 1
\end{bmatrix}\]
...
\[r_1(x) = \begin{bmatrix}
    2\\
    0
\end{bmatrix}\cos x \implies \overrightarrow{y_p} = \overrightarrow{U}\cos x + \overrightarrow{V}\sin x\]
\[\begin{bmatrix}
    0\\
    3
\end{bmatrix}e^x \implies \overrightarrow{y_p} = \overrightarrow{W}e^x\]
\[\begin{bmatrix}
    1\\
    -1
...
\end{bmatrix}x + \begin{bmatrix}
    0\\
    1
\end{bmatrix} \implies \overrightarrow{y_p} = \overrightarrow{C}x^2 + \overrightarrow{D}x + \overrightarrow{U}\]
\textbf{Solution.} Solve the IVP 
...
\end{bmatrix}\overrightarrow{y} + \begin{bmatrix}
    9x - 51\\
    7 + e^{2x}
\end{bmatrix}; \ \overrightarrow{y}(0) = \begin{bmatrix}
    1\\
    0
\end{bmatrix}\]
...
\[\overrightarrow{\rho} = \begin{bmatrix}
    -3t - 1/2\\
    t
\end{bmatrix}\]
We'll take $t= 0$, 
...
\begin{align*}
    \overrightarrow{y}_H &= c_1\begin{bmatrix}-3\\1\end{bmatrix}xe^{3x} + c_2\left(x\begin{bmatrix}
        -3\\
        1
    \end{bmatrix} + \begin{bmatrix}
        -1/2\\
